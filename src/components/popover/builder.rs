...
use super::{PopoverContentMarker, PopoverContentStyle, PopoverMarker, PopoverState, PopoverTriggerMarker};
...
impl PopoverBuilder {
    pub fn new(trigger: impl Into<String>) -> Self {
        Self { trigger: trigger.into(), content: None, open: false }
    }
...
impl<'w, 's> UiBuilder<'w, 's> for PopoverBuilder {
    ...
    fn spawn(self, parent: &'s mut ChildSpawnerCommands<'w>, theme: &UiTheme, font: &Handle<Font>) -> Self::Output {
        let mut root = parent.spawn((PopoverMarker, PopoverState { open: self.open }, Name::new("Popover")));
