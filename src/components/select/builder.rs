...
use super::{
    ...
    style::{
        SelectContentStyle, SelectOptionStyle, SelectRootStyle, SelectTriggerStyle,
    },
};
...
impl Default for SelectBuilder {
    fn default() -> Self {
        Self { options: Vec::new(), selected: None, width: None, open: false }
    }
}
...
impl<'w, 's> UiBuilder<'w, 's> for SelectBuilder {
    ...
    fn spawn(self, parent: &'s mut ChildSpawnerCommands<'w>, theme: &UiTheme, font: &Handle<Font>) -> Self::Output {
        ...
        if let Some(w) = self.width { root_style.node.width = w; }
        let mut root = parent.spawn((SelectMarker, root_style, SelectState { open: self.open, selected: self.selected.clone() }, Name::new("Select")));
        ...
        root.with_children(|rc| {
            ...
            .with_children(|tc| {
                let display = self
                    .selected
                    .as_ref()
                    .and_then(|v| self.options.iter().find(|(val, _)| val == v).map(|(_, l)| l.clone()))
                    .unwrap_or_else(|| "Select".to_string());
                let text_entity = LabelBuilder::new(display).spawn(tc, theme, font);
                tc.commands().entity(text_entity).insert(SelectTriggerTextMarker);
            });
            ...
            let mut content = rc.spawn((
                ...
                if self.open { Visibility::Inherited } else { Visibility::Hidden },
            ));
            content.with_children(|cc| {
                for (value, label_text) in self.options {
                    cc.spawn((
                        SelectOptionMarker { value: value.clone(), label: label_text.clone() },
                        SelectOptionStyle::new(theme),
                        Interaction::default(),
                        FocusPolicy::Block,
                    ))
                    .with_children(|oc| {
                        LabelBuilder::new(label_text).spawn(oc, theme, font);
                    });
                }
            });
        });
